  ...
  onMount(() => {
    ...
    autocomplete({
      ...
      fetch: function (text, callback) {
        fetch(
          `https://nominatim.openstreetmap.org/?&q=${text}&format=json&limit=5`
        )
    ...
    function warpScale() {
      ...
      let metersPerPx =
        (156543.03392 * Math.cos((ll[1] * Math.PI) / 180)) / Math.pow(2, z);
...
<div id="map-view">
      ...
      <div class="block">
        ...
        <button
          ...
          on:click={() => {
            rotation = rotation - 10;
            warpImage();
          }}>⤴️ Rotate Counterclockwise</button
        >
      </div>
    </div>
    <input
  ...
  <div>
    <button class="button my-2 is-danger" on:click={startOver}
      >Choose a new image</button
    >
  </div>
